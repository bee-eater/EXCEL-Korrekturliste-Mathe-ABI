...
Public Function CreateResults()
    ...
    If makeSure Then
        ...
        End If
        Worksheets.Add(After:=Worksheets(WbNameConfig)).Name = WbNamePrintSheet
        ...
        With Worksheets(WbNamePrintSheet).PageSetup
            ...
            .FitToPagesWide = 1
            .PrintArea = "A1:Q" & CStr(5 + (sheetCnt * 4) + (gNumOfPupils - 1) * ((4 * (sheetCnt + 1)) + 2))
...
Private Function PaintPrintPage(sheetCnt As Integer)
    ...
    Worksheets(WbNamePrintSheet).Columns(1).ColumnWidth = 13.14
    For i = 0 To 15
